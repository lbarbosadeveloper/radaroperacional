...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ============================
 // ============================
// ✅ CLIMA (robusto com cache local + timeout)
// ============================
...
function renderWeather(j, { stale = false } = {}) {
  ...
  // mantém wTemp vazio (como você quer)
  safeSet(els.wTemp, "");
  ...
  if (els.wUpdated) {
    ...
    els.wUpdated.textContent = when;
  }
}

async function loadWeather() {
  // 1) pinta algo imediatamente (último cache, se existir)
  ...
  try {
    ...
    if (!res.ok) {
      const txt = await res.text().catch(() => "");
      throw new Error(`Falha no /weather HTTP ${res.status}: ${txt.slice(0, 150)}`);
    }

    const j = await res.json();
    if (!j?.ok) throw new Error(j?.error || "Resposta inválida do /weather");

    // salva + renderiza fresco
    saveLastWeather(j);
    ...
    // 3) se nem cache tem, aí sim mostra placeholder
    if (!last) {
      renderWeather(
        { place: "Água Santa • RJ", cond: "—", min: null, max: null },
        { stale: true }
      );
    }
  }
